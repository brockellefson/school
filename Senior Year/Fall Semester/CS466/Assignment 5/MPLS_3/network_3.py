...
class MPLSFrame:
    ...
    ##@param label: label of MPLS Frame
...
class NetworkPacket:
    ...
    def from_byte_S(self, byte_S):
        ...
        priority = byte_S[NetworkPacket.dst_S_length : NetworkPacket.dst_S_length + NetworkPacket.p_S_length]
        data_S = byte_S[NetworkPacket.dst_S_length + NetworkPacket.p_S_length : ]
...
class Router:
    ...
    def process_queues(self):
        for i in range(len(self.intf_L)):
            in_queue = None #make sure we are starting the loop with a blank frame
            in_queue = self.intf_L[i].in_queue #get frame from interface i
            if in_queue.empty():
                continue # no frame to process yet
            fr_S = ''
            if in_queue.qsize() == 1:
                fr_S = in_queue.get(False)
            else:
                queuelist = list(self.intf_L[i].in_queue.queue)
    ...
    def process_network_packet(self, pkt, i):
        la = self.encap_tbl_D.get(int(pkt.priority))
        m_fr = MPLSFrame(pkt.dst + 'Frme' + str(la), pkt.to_byte_S())
    ...
    def process_MPLS_frame(self, m_fr, i):
        ...
        j = self.frwd_tbl_D.get(i)
        if j is None: #the MPLS packet is not in table therefore decapsolate
            pkt = NetworkPacket.from_byte_S(m_fr.data_S)
            j = self.decap_tbl_D.get(i)
            self.send(pkt, 'Network', j) #send network packet to host
        else:
            self.send(m_fr, 'MPLS', j) #forward MPLS packet to router

    def send(self, msg, link, i):
        try:
            fr = LinkFrame(link, msg.to_byte_S())
            self.intf_L[i].put(fr.to_byte_S(), 'out', True)
