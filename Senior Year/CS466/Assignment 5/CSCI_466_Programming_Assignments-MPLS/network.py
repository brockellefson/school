...
class Router:
    ...
    def process_MPLS_frame(self, m_fr, i):
        ...
        i = self.frwd_tbl_D.get(m_fr.label)
        if i is None: #the MPLS packet is not in table therefore decapsolate
            pkt = NetworkPacket.from_byte_S(m_fr.data_S)
            i = self.frwd_tbl_D.get(pkt.dst)
            self.send(pkt, 'Network', i) #send network packet to host
        else:
            self.send(m_fr, 'MPLS', i) #forward MPLS packet to router
